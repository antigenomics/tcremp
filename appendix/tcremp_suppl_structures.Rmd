...
```{r}
roundfactor <- 2.0
struct_alns_rmsd.mhc.dist |>
  melt(id.vars = 1:5,
       variable.name = "region",
       value.name = "distance") |>
  filter(clone_id.from > clone_id.to) |>
  mutate(chain = substr(region, 8, 8),
         rmsd = round(rmsd * roundfactor) / roundfactor) |>
  mutate(rmsd = ifelse(rmsd >= 5, "5+", signif(rmsd, 1)) |>
           as.factor()) |>
  ggplot(aes(x = rmsd, 
             group = rmsd, 
             y = distance)) +
  geom_boxplot() +
  facet_wrap(chain ~ region, scales = "free") +
  theme_minimal() +
  theme(aspect.ratio = 1)
```
